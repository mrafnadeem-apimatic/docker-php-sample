...
| `payPalMockResponse` | [`?PaypalMockResponseSchema`](../../doc/models/paypal-mock-response-schema.md) | Header, Optional | Header to simulate negative responses |
...
$result = $ordersController->ordersCreate(
    $body,
    $payPalRequestId,
    null,
...
```php
function ordersGet(string $id, ?string $fields = null): Order
```
...
$result = $ordersController->ordersGet($id);
```
...
```php
function ordersPatch(string $id, ?array $body = null): void
```
...
$result = $ordersController->ordersConfirm(
    $id,
    null,
    $prefer,
    $body
...
$result = $ordersController->ordersAuthorize(
    $id,
    null,
    $prefer
);
...
$result = $ordersController->ordersCapture(
    $id,
    null,
    $prefer
);
...
```php
function ordersTrackCreate(string $id, OrderTrackerRequest $body, ?string $payPalAuthAssertion = null): Order
```
...
$result = $ordersController->ordersTrackCreate(
    $id,
    $body
);
```
...
```php
function ordersTrackersPatch(string $id, string $trackerId, ?array $body = null): void
```
...
$ordersController->ordersTrackersPatch(
    $id,
    $trackerId,
    $body
);
